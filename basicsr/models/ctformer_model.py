...
class CTformerModel(BaseModel):
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                arrs = split_arr(self.lq, 64).to(self.device)
                arrs[0:64] = self.net_g(arrs[0:64])
                arrs[64:2 * 64] = self.net_g(arrs[64:2 * 64])
                arrs[2 * 64:3 * 64] = self.net_g(arrs[2 * 64:3 * 64])
                arrs[3 * 64:4 * 64] = self.net_g(arrs[3 * 64:4 * 64])
                self.output = agg_arr(arrs, 512).to(self.device)
        else:
            self.net_g.eval()
            with torch.no_grad():
                arrs = split_arr(self.lq, 64).to(self.device)
